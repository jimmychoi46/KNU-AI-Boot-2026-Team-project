...
- **검증 이중 방어**: 쓸 때(`save_subscription`)도 검증하고, 읽을 때(`load_subscriptions`)도 잘못된 행은 그 행만 건너뜁니다. 한 명의 잘못된 값이 그 시각 전체 발송을 막지 않습니다. 저장 시 길이·개수 상한도 둡니다(이메일 254자, 이름 100자, 키워드 최대 50개·각 50자) — 값의 크기 자체에는 별도 제한이 없어, 과도하게 큰 입력으로 저장소가 부풀지 않도록 막는 장치입니다.
...
- **발송 시각 규칙**: `send_hour`는 0~24, `send_minute`은 30분 단위(0 또는 30)입니다. `24`는 자정(다음 날 0시)을 뜻하며, 발송 요일 판정도 다음 날 기준으로 처리합니다. 시각은 정수만 받습니다 — `5.5` 같은 소수는 400을 반환합니다(예전엔 소수가 `int()`로 조용히 잘려 엉뚱한 시각에 저장될 수 있었습니다). 규칙을 어기면 400을 반환합니다(생략하면 필수값 누락으로 422).
