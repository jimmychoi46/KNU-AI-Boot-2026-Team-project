...
- **검증 이중 방어**: 쓸 때(`save_subscription`)도 검증하고, 읽을 때(`load_subscriptions`)도 잘못된 행은 그 행만 건너뜁니다. 한 명의 잘못된 값이 그 시각 전체 발송을 막지 않습니다. 저장 시 길이·개수 상한도 둡니다(이메일 254자, 이름 100자, 키워드 최대 50개·각 50자). 수정(PUT)·시드 가져오기는 속도 제한이 없어, 과도하게 큰 입력으로 저장소가 부풀지 않도록 막는 장치입니다.
