...
def _rate_key_create(request):
    ...
    [남는 한계 — 외부 배포 시 보강] 이 키는 정규화된 '한 주소'당 시간당 5건까지만 막는다.
    (1) Gmail 별칭(victim+1@…, v.i.c.t.i.m@…)은 실수신함이 같아도 서로 다른 키라 한 실사용자에게
    5건을 넘겨 보낼 수 있고, (2) 전역/소켓 IP 상한이 없어 서로 다른 주소를 계속 바꿔 넣으면
    한 IP 에서 확인 메일 발송량 자체는 무제한이다. 프론트가 신뢰 프록시(고정 IP)이거나 프론트–백엔드
    사이에 API 키가 있는 배포 환경에서 전역 IP 상한을 함께 걸어야 완전히 막힌다. 지금은 각자 로컬
    실행이라 해당 위협이 없어 보강을 미뤄 둔다."""
