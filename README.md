...
- 키워드는 구독자가 자유롭게 입력하도록 구현하였습니다. 백엔드는 키워드 저장 시 정제(공백/빈값/중복 정리 + 제어·보이지 않는 유니코드 문자 제거) 후 저장합니다.
...
- **검증 이중 방어**: 쓸 때(`save_subscription`)도 검증하고, 읽을 때(`load_subscriptions`)도 잘못된 행은 그 행만 건너뜁니다. 한 명의 잘못된 값이 그 시각 전체 발송을 막지 않습니다. 저장 시 길이·개수 상한도 둡니다(이메일 254자, 이름 100자, 키워드 최대 50개·각 50자). 업무상 규칙이라기보다, 과도하게 큰 입력이 저장소를 불필요하게 키우는 것을 막기 위한 안전장치입니다.
...
정기 발송은 수집 → 요약 → 발송을 한 번에 실행하지 않고, SQLite DB를 사이에 둔 독립 배치 작업 3개로 나눕니다. 각 작업은 서로 다른 주기로 실행될 수 있고, 단계 결과는 DB로 넘어갑니다.
...
- **환각 방지(원문에 없는 내용 차단)**: 자동 발송이라 사람이 매번 검수할 수 없으므로, 요약이 원문에 없는 내용을 지어내는 **환각**을 두 겹으로 막습니다. (1) 요약에 원문 기사에 없는 숫자(등장하지 않은 통계·지수 등)가 섞이면 그 항목을 코드가 제거합니다 — 링크 무결성 검사가 원문에 없는 링크를 걸러내는 것과 같은 방식의 결정적 검사입니다. (2) 초안을 다시 원문과 대조해 근거 없는 문장을 덜어냅니다. 틀린 수치·사실을 보내느니 그 항목을 누락시키는 쪽을 택합니다.
...
- **근접 중복(같은 안건)까지 뺀다**: URL이 달라도 제목+본문 스니펫이 거의 같은 전재·경미 수정 기사도 같은 것으로 보고 함께 뺀다(`db.fetch_seen_or_similar`). 기사마다 제목+본문 스니펫(네이버 검색 API `description`, LLM 요약이 아님)의 SimHash(문자 3-gram·64비트, blake2b라 실행마다 값이 같다)를 미리 저장해 두고(`articles.simhash`·`sent_articles.simhash`), 이미 받은 기사와의 Hamming 거리가 `NEAR_DUP_HAMMING_MAX`(기본 12) 이하면 같은 안건으로 본다. 임계값을 작게 둬 서로 다른 안건을 잘못 합치지 않고, 음수로 두면 근접 중복 판정은 끄고 완전 일치만 본다.
...
- **DB 스키마**: `subscribers`(구독자) · `articles`(정제 뉴스; 근접 중복 판정용 `simhash` 포함) → `digests`(조합별 요약 스냅샷; `latest_article_at`로 신선도 판정) → `digest_issues`(헤드라인) → `digest_topics`(주제+요약) → `digest_links`(관련 기사) · `sent_articles`(구독자별 재발송 방지 기록; `simhash` 포함).
...
`uvicorn src.api:app --reload`로 띄우고 `/docs`의 Swagger UI에서 바로 테스트할 수 있습니다. 저장·검증은 `subscriptions.py`(→ `db.py`)를 재사용하며, 검증 실패는 400으로 응답합니다. 대부분의 엔드포인트에 속도 제한이 걸려 있어, 초과하면 아래 표에 없는 **429**가 반환될 수 있습니다(선택지 조회·확인 안내 페이지 등은 예외). 일반 엔드포인트는 요청 IP 기준으로 제한하지만, **개인을 겨냥하는 엔드포인트**(본인 확인 코드 발송, `/subscribers/{email}` 조회·수정·삭제)는 **경로의 이메일 주소 기준**으로 제한합니다. 프론트(Streamlit)가 백엔드를 서버 대 서버로 호출해 모든 사용자의 IP가 동일하게 찍히므로, IP 기준만으로는 (1) 한 사용자가 전체 한도를 소진시키거나 (2) 여러 IP로 분산한 확인 코드 무차별 대입을 막지 못하기 때문입니다. 이메일을 키로 쓰면 두 경우가 모두 막히고, 대소문자만 바꾼 우회도 같은 주소로 정규화해 함께 걸립니다.
...
| `PUT` | `/subscribers/{email}` | 구독자 정보 수정(전체 교체) (관리자 또는 본인) | 인증 실패 401, 없으면 404, 값 오류 400, 바꾸려는 이메일이 이미 있으면 409 |
| `DELETE` | `/subscribers/{email}` | 구독 취소 (관리자 또는 본인) | 인증 실패 401, 없으면 404 |

- **이메일 확인**: `POST /subscribers`는 구독자를 `confirmed=False`로 저장하고, 확인 메일을 보냅니다. 그 링크(`GET /confirm`)를 열면 확인 안내 페이지만 뜨고(링크 사전열람만으론 확정되지 않게 함), 그 페이지의 버튼이 보내는 `POST /confirm`이 `confirmed=True`로 확정해 정기 발송 대상이 됩니다. 토큰은 1회용이라 확인 후 폐기되며, 재사용하면 400을 반환합니다. 이미 확인된 이메일로 다시 신청하면 409를 반환하지만, 아직 미확인인 이메일로 재신청하면 409 대신 확인 메일을 재전송합니다(같은 토큰 재사용).
- **이메일 바꾸기**: 수정 요청 본문의 `email`을 지금과 다르게 넣으면 이메일 변경으로 처리됩니다(PUT 한 번이면 됩니다). 새 주소는 다시 미확인 상태가 되어 확인 메일을 새로 받습니다(주인이 바뀌었으니까요). 단, 바꾸려는 새 주소가 이미 등록돼 있으면 — 그 주소가 아직 확인 전이더라도 — 409로 막습니다. 남이 가입해 확인을 기다리는 주소를 소리 없이 덮어써 없애 버리지 않도록 하기 위함입니다.
- **발송 시각 규칙**: `send_hour`는 0~24, `send_minute`은 30분 단위(0 또는 30)입니다. `24`는 자정(다음 날 0시)을 뜻하며, 발송 요일 판정도 다음 날 기준으로 처리합니다. 시각은 정수만 받습니다 — `5.5` 같은 소수는 요청 검증(Pydantic)에서 걸러져 422를 반환합니다(예전엔 소수가 `int()`로 조용히 잘려 엉뚱한 시각에 저장될 수 있었습니다). 범위(0~24)를 벗어나거나 30분 단위가 아니면 백엔드 검증에서 400을 반환합니다(시각 필드 자체를 생략하면 필수값 누락으로 422).
- **목록 나눠 받기(`GET /subscribers`)**: `?limit=&offset=`으로 목록을 나눠 받고, 전체 개수는 응답 헤더 `X-Total-Count`로 돌려줍니다. 개수와 목록 모두 검증을 통과한 구독자만 기준으로 세므로, 불량 데이터가 섞여 있어도 헤더 값과 실제 목록이 어긋나지 않습니다.
...
- `keywords`는 자유 입력입니다. 저장 시 공백을 정리하고 빈값·중복을 제거하며, 제어문자와 보이지 않는 유니코드 서식 문자(제로폭·양방향 등)도 함께 제거합니다. 후보 제한은 없습니다.
...
- **관리자 인증**: `GET /subscribers`는 관리자 전용입니다. `.env`의 `ADMIN_PASSWORD`와 같은 값을 `X-Admin-Password` 헤더로 보내야 합니다. 헤더가 없거나 값이 틀리면 401을 반환합니다. Swagger UI에서는 우측 상단 Authorize 버튼에 비밀번호를 넣으면 이후 요청에 자동으로 실립니다. `ADMIN_PASSWORD`가 서버에 아예 설정되지 않았으면 어떤 값을 보내도 401을 반환합니다 — 설정 안 됨을 누구나 통과로 취급하지 않습니다.
- **본인 확인(셀프서비스)**: 가입 시 이메일 확인은 이메일 소유를 1번 확인할 뿐, 그 뒤 조회·수정·삭제 요청이 실제 그 이메일 주인이 보낸 건지는 증명하지 않습니다. 그래서 `GET`/`PUT`/`DELETE /subscribers/{email}`는 `X-Admin-Password` 또는 `X-Access-Code` 중 하나가 필요합니다. 후자는 `POST .../access-code`로 발급받아 이메일로 받는 코드이며, `ACCESS_CODE_TTL_MINUTES`(기본 15분) 동안 재사용할 수 있어 조회 후 수정처럼 API를 연달아 부르는 흐름도 지원합니다. 입력한 코드는 대소문자와 앞뒤 공백을 구분하지 않습니다.
