...
| 날짜 | 한 일 |
|---|---|
| 7/7 (화) | 파트별 기능 개발 병행 — LLM 멀티에이전트 · 백엔드 파이프라인 · 프론트 대시보드 |
| 7/8 (수) | 프론트엔드 완성 · 백엔드 연동 (대시보드 UI 완성 후 REST API 연동 시작) |
| 7/9 (목) | 통합 테스트 · 버그 수정 (숫자 환각 방지 강화 · 24시 자정 처리 · 재발송 방지 강화) |
| 7/10 (금) | 시연 준비 · 문서화 (데모 시나리오 구성 · 시연 가이드 작성) |
| 7/13 (월) | 발표 자료 제작 · 리허설 (예상 질문 정리 · 발표 리허설) |
| 7/14 (화) | 최종 발표 · Q&A |
...
**장점**
- 관심 키워드·받는 시각·주기·요약 길이·언어까지 사용자별로 맞춤 발송
- 요약을 요약·검수 두 단계로 나눠, 초안을 다시 한 번 검수한 뒤 발송
- 원문에 없는 숫자·링크는 코드가 결정적으로 걸러냄 (LLM 요약을 코드로 재검증)
- 링크가 달라도 내용이 거의 같은 전재 기사까지 같은 것으로 보고 중복 제거

**알려진 제약·가정**
- 단일 컴퓨터에서 SQLite로 실행 → 배포는 간단하지만 확장성은 제한적
- 의심스러운 항목은 아예 제외 → 정확성은 높지만 정보가 일부 누락될 수 있음
- 환각 방어는 완전 차단이 아니라 확률적 억제까지 포함 → 숫자·링크만 코드가 결정적으로 차단
- 방어 효과를 정량 평가셋으로 아직 측정하지 못함
- 발송 작업에 여유 시간을 뒀지만, 시스템 절전 등으로 1분 이상 중단됐다 재개되면 그 시각 발송이 그날 누락될 수 있음
- 공개 환경에 배포하면, 서버 대 서버 프론트엔드를 전제로 한 요청 제한을 IP 기준 등으로 보강해야 함
- 링크가 달라도 내용이 거의 같은 기사를 같은 것으로 보고 거를 때 글자 단위 유사도로 판정하므로, 기준값에 따라 일부를 놓치거나 서로 다른 기사를 과하게 하나로 묶을 수 있음

**미흡한 점 · 향후 개선 방향**
- 정해진 시각 발송에서 나아가, 중요한 속보가 뜨면 즉시 알리는 방식으로 확장
- 구독자가 실제로 읽고 반응한 기사 피드백으로 발송 우선순위를 학습
- 기사의 화제성·출처 신뢰도를 점수로 표시
- 발송 요일을 직접 선택하는 화면 제공
- 주간 키워드를 글자 단위가 아니라 의미가 비슷한 주제끼리 묶는 방식(클러스터링) 도입
- 방어 효과를 측정할 정량 평가셋 구축
