...
- **발송 시각 규칙**: `send_hour`는 0~24, `send_minute`은 30분 단위(0 또는 30)입니다. `24`는 자정(다음 날 0시)을 뜻하며, 발송 요일 판정도 다음 날 기준으로 처리합니다. 시각은 정수만 받습니다 — `5.5` 같은 소수는 400입니다(예전엔 소수가 `int()`로 조용히 잘려 엉뚱한 시각에 저장될 수 있었습니다). 규칙을 어기면 400(생략하면 필수값 누락으로 422).
