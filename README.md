...
수집·요약·발송을 한 번에 돌리지 않고, **SQLite를 사이에 둔 3단계 배치**로 나눴습니다. 무거운 수집·요약이 발송 시각을 밀지 않도록 단계를 분리하고, 각 단계 결과는 DB로 넘어갑니다. 프론트는 백엔드 REST API로 구독자를 관리합니다.
...
flowchart LR
    ...
    SCH["스케줄러<br/>(APScheduler)"] --> COL["① 수집 · 네이버 오픈API"]
    COL -->|"기사"| DB
    SCH --> SUM["② 요약 · 멀티에이전트 LLM"]
    DB -->|"기사"| SUM
    SUM -->|"요약"| DB
    SCH --> DIS["③ 발송 · Gmail SMTP"]
    DB -->|"요약"| DIS
    DIS --> MAIL["구독자 메일함"]
```

- **① 수집** — 구독 키워드의 뉴스를 네이버 오픈API(검색)로 모아 정제해 `articles`에 저장
- **② 요약** — (키워드·길이·언어) 조합마다 기사를 모아 **요약 에이전트 → 검수 에이전트**로 이슈→주제 단위 요약을 만들어 `digests`에 저장(환각·링크 무결성 검사 포함)
- **③ 발송** — 발송 시각이 된 구독자에게 최신 요약을 렌더링해 메일로 보내고, 보낸 기사를 기록해 다음엔 중복 발송하지 않음
